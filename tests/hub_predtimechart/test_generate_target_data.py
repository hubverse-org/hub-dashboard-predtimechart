...
# the 182 files can be seen in `test__generate_target_json_files_flu_metrocast()`
@pytest.mark.parametrize("is_regenerate,exp_num_files", [(True, 182), (False, 171)])
def test__generate_target_json_files_regenerate_flu_metrocast(is_regenerate, exp_num_files, tmp_path):
    """
    Tests `_generate_target_json_files()`'s `is_regenerate` arg by checking that, if set, it skips the 11 existing files
    copied to the output dir before being run. If `is_regenerate` is True then all 182 files should be created. O/w only
    182 - 11 = 171 should be created. We do not check file contents here - they are tested elsewhere - only numbers of
    files created.
    """
    hub_dir = Path('tests/hubs/flu-metrocast')
    hub_config = HubConfigPtc(hub_dir, hub_dir / 'hub-config/predtimechart-config.yml')
    ...
    # copy the 11 expected files to the output directory, which should be skipped if `is_regenerate` - see
    # parameterization
